...
pub fn canvasWndProc(hwnd: c.HWND, msg: c.UINT, wParam: c.WPARAM, lParam: c.LPARAM) callconv(.C) c.LRESULT {
    ...
    std.debug.print("---- canvasWndProc {any}]\n", .{msg});
    return switch (msg) {
        else => c.DefWindowProcA(hwnd, msg, wParam, lParam),    
    };
}

fn open_window(name : [:0]const u8, width : i32, height :i32) !void {
...
//   // Calculate the required size of the window rectangle based on desired client area size
    ...
    _ = c.ShowWindow(g_hwnd, c.SW_NORMAL);
...
fn fenster_loop() i32 {
  ...
  // request reqpaint
  _ = c.InvalidateRect(g_hwnd, null, c.TRUE);
  return 0;
}
...
pub fn main() !void {
    ...
    try open_window("AAAABCD", 800,600);

    while (fenster_loop() == 0) {
    }
